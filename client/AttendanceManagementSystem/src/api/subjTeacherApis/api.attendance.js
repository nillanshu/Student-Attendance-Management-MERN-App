...
export async function fetchClassAttendance(date) {
    const res = await axios.post('/user/subjTeacher/viewClassAttendance', 
        { dateTimeTaken: date },
        {
            headers: {
...
export async function fetchAllStudents() {
    const res = await axios.get('/user/subjTeacher/getAllStudents');
    if(res.status !== 200) {
        const error = new Error(res.error);
        throw error;
    }
...
export async function fetchStudentAttendance(studentAttnInfo) {
    const res = await axios.post('/user/subjTeacher/viewStudentAttendance', 
        { 
...
export async function loadTakeAttendancePage() {
    const res = await axios.post('/user/subjTeacher/loadTakeAttendancePage', {
        headers: {
            "Content-Type": "application/json"
        }
    });
...
export async function takeAttendance(check) {
    const res = await axios.patch('/user/subjTeacher/takeAttendance', 
        { 
            check: check,
        },
        {
...
export async function downloadAttendance() {
    try {
        const res = await axios.get('/user/subjTeacher/downloadAttendance', {
            headers: {
                "Content-Type": "application/json"
            },
            responseType: 'blob'
